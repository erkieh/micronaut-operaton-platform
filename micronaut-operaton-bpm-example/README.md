...
`git clone https://github.com/operaton-community-hub/micronaut-operaton-platform-7.git`
...
### IntelliJ IDEA
To run the application in IntelliJ IDEA simply expand the sub-module `micronaut-operaton-bpm-example` and start `info.novatec.micronaut.operaton.bpm.example.Application`.

### Unix/Mac Console
To run the application in a console on Unix/Mac:
```
./gradlew clean run -p micronaut-operaton-bpm-example
```

### Windows Command Line
To run the application in a Windows command line:
```
gradlew.bat clean run -p micronaut-operaton-bpm-example
```

### Docker
To run the application in Docker:
```
./gradlew clean dockerBuild -p micronaut-operaton-bpm-example
docker run -p 8080:8080 micronaut-operaton-bpm-example:latest
```
...
The Operaton Cockpit is available at `http://localhost:8080/`. Login with `admin`/`admin`.

The Operaton REST API is available at the context path `/engine-rest`, e.g. `GET http://localhost:8080/engine-rest/engine`.
