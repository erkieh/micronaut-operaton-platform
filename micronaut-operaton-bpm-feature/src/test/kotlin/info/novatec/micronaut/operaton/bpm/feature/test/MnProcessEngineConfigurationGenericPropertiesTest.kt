...
/**
 * Generic property tests for [info.novatec.micronaut.operaton.bpm.feature.MnProcessEngineConfiguration].
 *
...
class MnProcessEngineConfigurationGenericPropertiesTest {
    ...
    fun validGenericPropertyAsString() {
        val properties: Map<String, Any> = mapOf(
            "operaton.generic-properties.properties.history-cleanup-strategy" to "endTimeBased",
            "operaton.generic-properties.properties.batch-job-priority" to "30",
            "operaton.generic-properties.properties.default-number-of-retries" to "1",
            "operaton.generic-properties.properties.dmn-enabled" to "false", //primitive data type: boolean
        )
    ...
    fun validGenericPropertyOfTypeStringAsBoolean() {
        val properties: Map<String, Any> = mapOf(
            "operaton.generic-properties.properties.database-schema-update" to true,
        )
        ApplicationContext.run(EmbeddedServer::class.java, properties).use { embeddedServer ->
            val processEngineConfiguration = embeddedServer.applicationContext.getBean(MnProcessEngineConfiguration::class.java)
    ...
    fun camelCase() {
        val properties: Map<String, Any> = mapOf(
            "operaton.generic-properties.properties.historyCleanupStrategy" to "endTimeBased"
        )
        ApplicationContext.run(EmbeddedServer::class.java, properties).use { embeddedServer ->
            val processEngineConfiguration = embeddedServer.applicationContext.getBean(MnProcessEngineConfiguration::class.java)
            assertEquals("endTimeBased", processEngineConfiguration.historyCleanupStrategy)
    ...
    fun invalidGenericProperty() {
        val properties = Collections.singletonMap<String, Any>("operaton.generic-properties.properties.xyz", "1")
