 ...
 */
package info.novatec.micronaut.operaton.bpm.feature.initialization;
...
@Singleton
@Requires(property = "operaton.filter.create")
public class FilterAllTaskCreator implements ParallelInitializationWithProcessEngine {
    ...
    public FilterAllTaskCreator(Configuration configuration) {
        filterName = configuration.getFilter().getCreate()
                .orElseThrow(() -> new IllegalArgumentException("If property 'operaton.filter.create' is set it must have a value."));
    }
