...
public class JettyRuntimeMissing implements ParallelInitializationWithoutProcessEngine {
    ...
    public void execute() {
        if (webappsEnabled) {
            log.warn("Webapps are enabled via 'operaton.webapps.enabled' but they are not supported on the current server runtime. Please switch to Jetty, see https://github.com/camunda-community-hub/micronaut-camunda-platform-7#camunda-rest-api-and-webapps");
        }
        if (restEnabled) {
            log.warn("REST is enabled via 'operaton.rest.enabled' but it is not supported on the current server runtime. Please switch to Jetty, see https://github.com/camunda-community-hub/micronaut-camunda-platform-7#camunda-rest-api-and-webapps");
        }
    }
}
